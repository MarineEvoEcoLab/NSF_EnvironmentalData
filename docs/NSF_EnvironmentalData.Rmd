...
discharge_permits <- read.delim("D:/Puritz_Lab/NSF/NSF_EnvironmentalData/data/EF_NPDES.csv",sep=",") %>% filter(STATE_CODE %in% states)
...
leaflet(width="100%") %>% addTiles() %>% 
  ...
  addMarkers(data=discharge_permits,lng=~LONGITUDE,lat=~LATITUDE,
             popup=~paste(
